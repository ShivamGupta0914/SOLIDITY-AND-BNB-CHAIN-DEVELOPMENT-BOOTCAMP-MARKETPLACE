...
contract Airdrop is ERC20, Ownable {
  ...
  /**
    ...
    */
  constructor(
    Istaking _stakingContractAddress,
    uint256 _totalSupply,
    uint256 _airdropReward
  ) ERC20("AirdropRewardToken", "MTK") {
  ...
  function claimReward(address _user) public {
    require(
      block.timestamp > stakingContractAddress.getEndTimestamp(),
      "Staking period not ended yet"
    );
    require(!claimed[msg.sender], "Airdrop: Reward already claimed");
    require(
      stakingContractAddress.isWhitelistedUser(_user),
